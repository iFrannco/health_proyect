...
class Perfil extends BaseController
{
    ...
    {
        $admin = $this->obtenerAdministradorActual();

        return view('paciente/perfil/index', $this->layoutData() + [
            'title'            => 'Mi perfil',
            'usuario'          => $admin,
            'rolLabel'         => 'Administrador',
            'perfilDescripcion'=> 'Actualizá tus datos personales y gestioná la seguridad de tu cuenta administrativa.',
            'formRoutes'       => [
                'datos'    => route_to('admin_perfil_actualizar_datos'),
                'password' => route_to('admin_perfil_actualizar_password'),
            ],
    ...
    public function actualizarDatos()
    {
        ...
        $rules = [
            ...
            'dni'      => 'required|min_length[6]|max_length[20]|is_unique[users.dni,id,' . (int) $admin->id . ']',
            'email'    => 'required|valid_email|max_length[180]|is_unique[users.email,id,' . (int) $admin->id . ']',
        ...
        try {
            $adminActualizado = $this->perfilService->actualizarDatos((int) $admin->id, $payload);
        } catch (\Throwable $exception) {
            log_message('error', 'Error al actualizar perfil del administrador: {exception}', ['exception' => $exception]);
        ...
        $this->actualizarSesion($adminActualizado);
        session()->setFlashdata('success', 'Perfil actualizado correctamente.');

        return redirect()->route('admin_perfil_index');
    }
    ...
    public function actualizarPassword()
    {
        ...
        if (! password_verify($passwordActual, (string) $admin->password_hash)) {
        ...
        try {
            $this->perfilService->actualizarPassword((int) $admin->id, $passwordNueva);
        } catch (\Throwable $exception) {
            log_message('error', 'Error al actualizar contraseña del administrador: {exception}', ['exception' => $exception]);
    ...
    private function obtenerAdministradorActual(): User
    {
        $session = session();
        $userId  = $session->get('user_id');

        if ($userId !== null) {
            $admin = $this->userModel->findActivoPorRol((int) $userId, UserModel::ROLE_ADMIN);
            if ($admin instanceof User) {
                return $admin;
            }
        }

        $admin = $this->userModel->findPrimeroActivoPorRol(UserModel::ROLE_ADMIN);
        if ($admin === null) {
            throw new PageNotFoundException('No existen administradores activos configurados.');
        }
